...
- **Card grids never exceed 4 columns at any breakpoint** (`grid-cols-{n}`/`sm:`/`lg:` all capped at 4) — a row of card art wider than that reads as a wall of thumbnails rather than a scannable set. Responsive breakpoints may show fewer columns on narrow screens, never more than 4 on wide ones. Applies to any grid of `VisualCardTile`/`CardImage`/card-art tiles (e.g. `TopCardsSections.tsx`'s grid layout, `ChampionSynergy.tsx`'s New Releases and Level-print grids) — not to unrelated flex-wrap rows of badges/chips/text.
- **Material Design principles are a site-wide rule, not just for card tiles.** Any interactive element — buttons, chips, tabs, form controls, nav items, cards, panels — should get: a tonal fill for the selected/active/pressed state (not just a border- or text-color swap), a real elevation change on hover/focus/selection (`shadow-md`/`shadow-lg`, using `Panel.tsx`'s existing 0/1/2 elevation ladder where a component already sits on it), a smooth lift/press transition (`transition-all duration-200`, `hover:-translate-y-0.5`, `active:scale-[0.97]`) instead of an instant state snap, and a non-color affordance (icon, weight change, checkmark) so state is never conveyed by color alone. `ChampionSynergy.tsx`'s Level-print selector is the reference implementation for a selectable tile. This doesn't mandate a blanket rewrite of every existing component (`Button.tsx`, `Chip.tsx`, `Tabs.tsx`, `FormControl.tsx` still use flatter treatments) — but any new interactive UI should be built to this standard, and an existing component is worth upgrading to it when you're already touching that file for something else.

## Response style

- Write all chat responses in ASD-STE100 (Simplified Technical English). Use short sentences. Use one instruction or one fact per sentence. Use active voice. Use approved, simple words. Do not use jargon or synonyms for the same term. Do not use long noun strings or complex clauses.
