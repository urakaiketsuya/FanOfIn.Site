...
export interface LevelGoalConfig {
  ...
  /** How many opening-library cards a detected Fragmented Spirit is assumed to inspect while
   * resolving Glimpse 6 followed by drawing six. Six means no misses are bottomed; twelve is the
   * maximum search depth when every miss is bottomed. */
  fragmentedSpiritDepth?: number;
}
...
export interface LevelGoalAnalysis {
  ...
  detected: { directLevelCards: string[]; fractalPaymentCards: string[]; fractalCards: string[]; fragmentedSpirit: string | null };
}
...
export function computeLevelGoalAnalysis(main: Line[], material: Line[], cardsByName: ReadonlyMap<string, Card>, config: LevelGoalConfig): LevelGoalAnalysis {
  ...
  const fragmentedSpirit = material.map((line) => cardsByName.get(line.name)).find((card) => card?.level === 0 && /\bglimpse\s+6\b/i.test(card.effect ?? "") && /\bdraw\s+six\s+cards\b/i.test(card.effect ?? "")) ?? null;
  ...
  const spiritDepth = fragmentedSpirit ? Math.min(12, Math.max(6, Math.round(config.fragmentedSpiritDepth ?? 12))) : startingHandSize;
  const fragmentedSelectionApplied = Boolean(fragmentedSpirit && accelerantsNeeded === 1);
  // The maximum-depth shortcut is exact for the usual one-enabler route: keep the hit if it is in
  // the first six, or bottom all six misses and draw the next six. Multiple required hits need a
  // conditional keep/bottom model, so they deliberately retain natural access odds for now.
  const directCardsInspected = Math.min(deckSize, seenAtTarget + (accelerantsNeeded === 1 ? Math.max(0, spiritDepth - startingHandSize) : 0));
  ...
  const directProbability = accelerantsNeeded === 0 ? 1 : directResourceFeasible ? chance(deckSize, [{ copies: directCopies, required: accelerantsNeeded }], directCardsInspected) : 0;
  ...
  if (config.useDirectLevelUp) routes.push({ id: "direct", label: fragmentedSelectionApplied ? "Direct level-up + Fragmented Spirit" : "Direct level-up route", probability: directLines.length ? directProbability : null, status: accelerantsNeeded === 0 ? "not-needed" : directLines.length && directResourceFeasible ? "available" : "blocked", detail: accelerantsNeeded === 0 ? "No acceleration is needed for this target turn." : `${accelerantsNeeded} extra level-up effect${accelerantsNeeded === 1 ? "" : "s"} needed by turn ${config.targetTurn}; ${directCopies} matching copies and at least ${directReserveDemand} Reserve payment are required.${fragmentedSelectionApplied ? ` ${fragmentedSpirit!.name} is modeled at ${spiritDepth} opening cards inspected.` : fragmentedSpirit && accelerantsNeeded > 1 ? " Fragmented Spirit selection is not added because exact multi-hit keep/bottom sequencing is not yet modeled." : ""}`, bottleneck: !directLines.length ? "No self-contained ‘level up your champion’ card was detected." : !directAffordable ? `The cheapest detected effect costs Reserve ${directCost}, beyond the inferred timing.` : !directResourceFeasible ? `${accelerantsNeeded} enabler card${accelerantsNeeded === 1 ? "" : "s"} plus ${directReserveDemand} Reserve payments exceed the ${seenAtTarget}-card natural hand ceiling.` : `Finding ${accelerantsNeeded} direct level-up card${accelerantsNeeded === 1 ? "" : "s"} among ${directCardsInspected} inspected cards; actual hand/resource capacity remains ${seenAtTarget} cards before spending.` });
  ...
  if (config.useDirectLevelUp && accelerantsNeeded > 0 && directResourceFeasible && directCopies >= accelerantsNeeded) {
    for (const line of directLines.filter((entry) => entry.quantity < 4)) {
      const improved = chance(deckSize, [{ copies: directCopies + 1, required: accelerantsNeeded }], directCardsInspected);
  ...
  return { startingHandSize, naturalTurn, routes, suggestions, detected: { directLevelCards: directLines.map((line) => line.name), fractalPaymentCards: paymentLines.map((line) => line.name), fractalCards: fractalLines.map((line) => line.name), fragmentedSpirit: fragmentedSpirit?.name ?? null } };
}
