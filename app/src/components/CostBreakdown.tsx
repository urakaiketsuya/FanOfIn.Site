...
export default function CostBreakdown({ card, effectiveCost, compact = false }: { card: Card; effectiveCost?: number; compact?: boolean }) {
  ...
  return <details className={compact ? "mt-1 text-[10px]" : "rounded-lg border border-ctp-surface1 bg-ctp-base/35 p-3 text-xs"}>
    <summary className="flex min-h-11 cursor-pointer items-center gap-1 font-medium text-ctp-subtext1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ctp-blue/40"><span>Printed {card.cost_reserve}</span><span aria-hidden="true">→</span><span className={reduction > 0 ? "text-ctp-green" : "text-ctp-text"}>effective {chosen}</span></summary>
