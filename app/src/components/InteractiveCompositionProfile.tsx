  ...
  return <div data-component="InteractiveCompositionProfile" className="space-y-4">
    ...
    <div className="rounded-lg border border-ctp-surface1 bg-ctp-mantle p-4">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(14rem,0.65fr)_minmax(0,1.35fr)]">
        ...
        <div aria-live="polite">{active && <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-ctp-subtext0">{active} cards</p>}{matching.length > 0 ? <div className="grid grid-cols-2 gap-x-3 gap-y-5 sm:grid-cols-3 xl:grid-cols-4">{matching.map((line) => <VisualCardTile key={line.card} line={line} card={cardsByName.get(line.card)} unitPrice={undefined} priceTrend={undefined} simulatorEvidence={undefined} communityEntry={undefined} fields={CARD_FIELDS} />)}</div> : <p className="text-sm text-ctp-subtext1">No resolved cards in this category.</p>}</div>
      </div>
    </div>
  </div>;
}
