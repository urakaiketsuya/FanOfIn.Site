...
<p className="text-xs text-ctp-subtext0">{workspace.championName ?? "Unknown Champion"} · {workspace.main.reduce((sum, line) => sum + line.quantity, 0)} cards</p>
...
<Panel><details><summary className="cursor-pointer font-semibold text-ctp-text">Why these odds?</summary><div className="mt-3 space-y-3 text-sm"><Purpose label="Level acceleration" cards={levelAnalysis.detected.directLevelCards} detail="Used when it improves on natural materialization." /><Purpose label="Opening selection" cards={levelAnalysis.detected.fragmentedSpirit ? [levelAnalysis.detected.fragmentedSpirit] : []} detail="Glimpse is applied at its useful search depth." /><Purpose label="Fractal payment" cards={levelAnalysis.detected.fractalPaymentCards} detail="Counted as payment support, not extra levels." /></div></details></Panel>
</div>}
      {tab === "analyze" && <Panel className="mt-4"><details><summary className="cursor-pointer font-semibold text-ctp-text">{fastestCombo ? `${fastestCombo.combo.name}: 50% by turn ${fastestCombo.point!.turn}` : savedCombos.length ? `${savedCombos.length} saved scenarios` : "Save and compare scenarios"}</summary><div className="mt-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><label className="text-xs text-ctp-subtext0 lg:col-span-2">Combo name<input value={comboName} onChange={(event) => setComboName(event.target.value)} className="mt-1 block min-h-11 w-full rounded-md border border-ctp-surface1 bg-ctp-base px-3 text-base text-ctp-text sm:text-sm" /></label><label className="text-xs text-ctp-subtext0">Combo damage<input type="number" min={0} value={comboDamage} onChange={(event) => setComboDamage(Number(event.target.value))} className="mt-1 block min-h-11 w-full rounded-md border border-ctp-surface1 bg-ctp-base px-3 text-base text-ctp-text sm:text-sm" /></label><label className="text-xs text-ctp-subtext0">Play order<select value={forecastOrder} onChange={(event) => setForecastOrder(event.target.value as "first" | "second")} className="mt-1 block min-h-11 w-full rounded-md border border-ctp-surface1 bg-ctp-base px-3 text-base text-ctp-text sm:text-sm"><option value="first">First</option><option value="second">Second</option></select></label><label className="text-xs text-ctp-subtext0">Lethal at<input type="number" min={1} value={lethalThreshold} onChange={(event) => setLethalThreshold(Math.max(1, Number(event.target.value)))} className="mt-1 block min-h-11 w-full rounded-md border border-ctp-surface1 bg-ctp-base px-3 text-base text-ctp-text sm:text-sm" /></label><button type="button" disabled={currentRecipe.length < 2} onClick={saveCurrentCombo} className="min-h-11 rounded-md bg-ctp-mauve px-3 text-sm font-semibold text-ctp-base disabled:opacity-40 sm:col-span-2 lg:col-span-5">Save current combo</button></div>{comboForecasts.length > 0 && <div className="mt-4 grid gap-2">{comboForecasts.map(({ combo, points }) => { const fifty = points.find((point) => (point.probability ?? 0) >= 0.5); return <article key={combo.id} className="rounded-lg border border-ctp-surface1 bg-ctp-base/40 p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-medium text-ctp-text">{combo.name}</p><p className="mt-1 text-xs text-ctp-subtext0">{fifty ? `50% by turn ${fifty.turn}` : "Below 50% through turn 6"}</p></div>{combo.damage >= lethalThreshold && <span className="rounded bg-ctp-red/15 px-1.5 py-0.5 text-[9px] font-bold text-ctp-red">LETHAL {combo.damage}</span>}</div><div className="mt-3 grid grid-cols-6 gap-1">{points.map((point) => <div key={point.turn} className="rounded bg-ctp-mantle px-1 py-2 text-center"><p className="text-[9px] text-ctp-subtext0">T{point.turn}</p><p className="mt-0.5 text-xs font-semibold tabular-nums text-ctp-text">{point.probability === null ? "—" : `${(point.probability * 100).toFixed(0)}%`}</p></div>)}</div><div className="mt-3 flex gap-4"><button type="button" onClick={() => { setLocalPreset({ key: `saved:${combo.id}`, label: combo.name, requirements: combo.requirements }); setCurrentRecipe(combo.requirements); setComboName(combo.name); setComboDamage(combo.damage); }} className="min-h-9 text-sm font-semibold text-ctp-blue">Open</button><button type="button" onClick={() => setSavedCombos((current) => current.filter((candidate) => candidate.id !== combo.id))} className="min-h-9 text-sm text-ctp-red">Remove</button></div></article>; })}</div>}</div></details></Panel>}
      {tab === "analyze" && levelAnalysis && goalId === "level" && <>
<details className="mt-4 rounded-xl border border-ctp-surface1 bg-ctp-mantle p-4"><summary className="cursor-pointer font-semibold text-ctp-text">Level route details</summary><div className="mt-4 grid gap-3 lg:grid-cols-3">{levelAnalysis.routes.map((route) => <Panel key={route.id} padding="sm">
...
</Panel>)}</div></details>{levelAnalysis.suggestions.length > 0 && <Panel className="mt-4">
...
<PackageArtStrip names={[entry.anchor, ...entry.members]} />
<div className="mt-3 flex flex-wrap gap-2 text-[11px] text-ctp-subtext0"><span>{entry.matchingDecks.toLocaleString()} decks</span><span>·</span><span>{included}/{entry.members.length + 1} in your deck</span></div>
...
function PackageArtStrip({ names }: { names: string[] }) {
  const visibleNames = useMemo(() => names.slice(0, 5), [names]);
  const cardsByName = useCardsByNames(visibleNames);
  return <div className="mt-3 grid h-28 grid-cols-5 gap-1.5 overflow-hidden rounded-lg bg-ctp-mantle p-1.5" aria-label="Package cards">
    {visibleNames.map((name) => {
      const card = cardsByName.get(name);
      return card?.editions[0]?.image ? <Link key={name} to={`/cards/${card.slug}`} title={name} className="min-w-0 overflow-hidden rounded-md"><CardImage image={card.editions[0].image} alt={name} className="h-full w-full object-cover object-top" /></Link> : <div key={name} title={name} className="flex min-w-0 items-center justify-center rounded-md bg-ctp-surface0 p-1 text-center text-[9px] text-ctp-subtext0">{name}</div>;
    })}
  </div>;
}
