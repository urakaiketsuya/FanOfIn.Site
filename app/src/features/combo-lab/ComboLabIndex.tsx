...
export default function ComboLabIndex() {
  ...
  const matchingAddableCards = useMemo(() => {
    const query = cardToAdd.trim().toLowerCase();
    if (!query || addableCardNames.includes(cardToAdd)) return [];
    return addableCardNames.filter((name) => name.toLowerCase().includes(query)).slice(0, 10);
  }, [addableCardNames, cardToAdd]);
  const selectedAddableCard = addableCardNames.includes(cardToAdd) ? cardToAdd : "";
...
</div>}</div></div>{editingDeck && <div className="mt-3 border-t border-ctp-surface1 pt-3"><div className="grid max-h-64 gap-1 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3">{workspace.main.map((line) => <div key={line.name} className="flex items-center justify-between rounded-md bg-ctp-base/50 px-2 py-1.5 text-xs"><span className="truncate text-ctp-text">{line.name}</span><span className="ml-2 flex items-center gap-1"><button type="button" aria-label={`Remove one ${line.name}`} onClick={() => updateMainQuantity(line.name, line.quantity - 1)} className="h-7 w-7 rounded border border-ctp-surface1">−</button><span className="w-5 text-center tabular-nums">{line.quantity}</span><button type="button" aria-label={`Add one ${line.name}`} disabled={line.quantity >= 4} onClick={() => updateMainQuantity(line.name, line.quantity + 1)} className="h-7 w-7 rounded border border-ctp-surface1 disabled:opacity-35">+</button></span></div>)}</div><div className="mt-2 flex items-start gap-2"><div className="relative min-w-0 flex-1"><input type="search" value={cardToAdd} onChange={(event) => setCardToAdd(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && matchingAddableCards[0]) { event.preventDefault(); setCardToAdd(matchingAddableCards[0]); } }} placeholder="Search for a card to add…" aria-label="Search for a card to add" autoComplete="off" className="min-h-10 w-full rounded-md border border-ctp-surface1 bg-ctp-base px-3 text-sm text-ctp-text focus:border-ctp-blue focus:outline-none focus-visible:ring-2 focus-visible:ring-ctp-blue/30" />{matchingAddableCards.length > 0 && <div className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-ctp-surface1 bg-ctp-base p-1 shadow-xl">{matchingAddableCards.map((name) => <button key={name} type="button" onClick={() => setCardToAdd(name)} className="block min-h-10 w-full rounded-md px-3 py-2 text-left text-xs text-ctp-text hover:bg-ctp-surface0 focus:bg-ctp-surface0 focus:outline-none">{name}</button>)}</div>}</div><button type="button" disabled={!selectedAddableCard} onClick={() => { updateMainQuantity(selectedAddableCard, 1); setCardToAdd(""); }} className="min-h-10 rounded-md bg-ctp-blue px-3 text-xs font-semibold text-ctp-base disabled:opacity-40">Add</button></div></div>}{notice && <p className="mt-2 text-xs text-ctp-green">{notice}</p>}</Panel>
      <div className="mt-4">
