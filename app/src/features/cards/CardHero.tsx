...
function Stat({ label, value, icon }: { label: string; value: number | string | null; icon?: ReactNode }) {
  ...
  return <div className="min-w-20 rounded-xl bg-ctp-surface1/70 px-4 py-2.5 text-center"><div className="flex items-center justify-center gap-1 text-[11px] font-medium uppercase tracking-wide text-ctp-subtext0">{icon}{label}</div><div className="mt-0.5 text-xl font-bold leading-none text-ctp-text">{value}</div></div>;
}
...
export default function CardHero({ card, edition, editionIndex, editionsExpanded, price, priceSeries, rarityLabel, onEditionChange, onEditionsExpandedChange }: { card: Card; edition?: CardEdition; editionIndex: number; editionsExpanded: boolean; price?: PriceRow; priceSeries: PriceSeries | null; rarityLabel: (rarity: number) => string; onEditionChange: (index: number) => void; onEditionsExpandedChange: (expanded: boolean) => void }) {
  return <Panel elevation={1} padding="lg" className="mt-4 overflow-hidden"><div className="grid grid-cols-1 gap-8 md:grid-cols-[280px_1fr]">
    ...
    <div className="min-w-0"><h1 className="text-3xl font-bold tracking-tight text-ctp-blue sm:text-4xl">{card.name}</h1><div className="mt-2 flex flex-wrap gap-1.5">{card.classes.map((value) => <Badge key={value} to={`/cards?class=${encodeURIComponent(value)}`}><ClassIcon cardClass={value} size={14} />{value}</Badge>)}{card.types.map((value) => <Badge key={value} to={`/cards?type=${encodeURIComponent(value)}`}><TypeIcon type={typeIconKey(value, card.types)} size={14} />{value}</Badge>)}{card.subtypes.map((value) => <Badge key={value} to={`/cards?subtype=${encodeURIComponent(value)}`}>{value}</Badge>)}{card.elements.map((value) => <Badge key={value} to={`/cards?element=${encodeURIComponent(value)}`}><ElementIcon element={value} size={14} />{value}</Badge>)}</div>
      <div className="mt-5 flex flex-wrap gap-2"><Stat label="Memory" value={card.cost_memory} icon={<CostIcon kind="memory" size={12} />} /><Stat label="Reserve" value={card.cost_reserve} icon={<CostIcon kind="reserve" size={12} />} /><Stat label="Level" value={card.level} /><Stat label="Power" value={card.power} /><Stat label="Life" value={card.life} /><Stat label="Durability" value={card.durability} /></div>
