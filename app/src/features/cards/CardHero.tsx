...
export default function CardHero({ card, edition, editionIndex, editionsExpanded, price, priceSeries, rarityLabel, onEditionChange, onEditionsExpandedChange }: { card: Card; edition?: CardEdition; editionIndex: number; editionsExpanded: boolean; price?: PriceRow; priceSeries: PriceSeries | null; rarityLabel: (rarity: number) => string; onEditionChange: (index: number) => void; onEditionsExpandedChange: (expanded: boolean) => void }) {
  ...
  const alternateFaces = edition?.other_orientations ?? [];
  ...
  return <Panel padding="lg" className="mt-4 overflow-hidden"><div className="grid grid-cols-1 gap-6 md:grid-cols-[280px_1fr]">
    <div className="mx-auto w-full max-w-[280px] md:mx-0">{faceImage ? <CardImage image={faceImage} alt={`${faceName}${alternateFace ? " reverse face" : ""}`} className="aspect-[5/7] w-full rounded-xl border border-ctp-surface2 object-cover shadow-xl shadow-black/30" /> : <div className="flex aspect-[5/7] items-center justify-center rounded-lg border border-ctp-surface1 bg-ctp-mantle text-ctp-subtext0">No image</div>}
      {alternateFaces.length > 0 && <div className="mt-3 rounded-xl border border-ctp-surface1 bg-ctp-base p-1" role="group" aria-label="Card face"><div className="grid grid-cols-2 gap-1"><button type="button" aria-pressed={faceIndex === 0} onClick={() => setFaceIndex(0)} className={`min-h-11 rounded-lg px-3 text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-ctp-blue/40 ${faceIndex === 0 ? "bg-ctp-blue text-ctp-base" : "text-ctp-subtext1 hover:bg-ctp-surface0"}`}>Front</button><button type="button" aria-pressed={faceIndex === 1} onClick={() => setFaceIndex(1)} className={`min-h-11 rounded-lg px-3 text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-ctp-blue/40 ${faceIndex === 1 ? "bg-ctp-blue text-ctp-base" : "text-ctp-subtext1 hover:bg-ctp-surface0"}`}>Back</button></div><p className="px-2 pb-1 pt-2 text-center text-xs text-ctp-subtext0">{faceIndex === 0 ? card.name : alternateFaces[0].name}</p></div>}
