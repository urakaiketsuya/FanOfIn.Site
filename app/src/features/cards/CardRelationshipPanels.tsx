...
export function CardPlayedWithPanel({ cardName, deckCount, topCards, cardImages }: { cardName: string; deckCount?: number; topCards: TopCardsBySection; cardImages: Map<string, Card> }) {
  if (deckCount === undefined) return <CardTabSkeleton label="Loading commonly paired cards" />;
...
export function CardSynergyPanel({ cardName, cards, totalDecks, cardImages, loading = false }: { cardName: string; cards: CardImpactEntry[]; totalDecks: number; cardImages: Map<string, Card>; loading?: boolean }) {
  if (loading) return <CardTabSkeleton label="Loading win-rate synergy" />;
