...
export default function ImportByUser({ comparedKeys, onToggle }: { comparedKeys: Set<string>; onToggle: (deck: ComparedDeck) => void }) {
  ...
  async function search() {
    ...
    setDeckError(null);
    try {
      const response = await accountApi.discoverProfiles(query.trim());
      if (request !== requestRef.current) return;
      setUsers(response.profiles); setSelectedUser(null); setResults([]); setStatus("ready");
    } catch { if (request === requestRef.current) setStatus("error"); }
  }

  async function selectUser(profileSlug: string) {
    const request = ++requestRef.current;
    setSelectedUser(profileSlug); setStatus("loading"); setDeckError(null);
    try {
      const { profile } = await accountApi.publicProfile(profileSlug);
      if (request !== requestRef.current) return;
      setResults(profile.decks); setStatus("ready");
    } catch { if (request === requestRef.current) setStatus("error"); }
  }
  ...
  async function toggle(summary: PublicDeckSummary) {
    ...
    catch { setDeckError("That public deck could not be loaded. It may no longer be shared."); }
  ...
  return <div data-component="ImportByUser">
    ...
    <div aria-live="polite">
      ...
      {status === "error" && <InlineState tone="danger" className="mt-3">User search is unavailable. Check your connection and try again.</InlineState>}
      {deckError && <InlineState tone="danger" className="mt-3">{deckError}</InlineState>}
    </div>
    ...
    {!selectedUser && users.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{users.map((user) => <button key={user.profileSlug} type="button" onClick={() => void selectUser(user.profileSlug)} className="min-h-10 rounded-md border border-ctp-surface1 px-3 py-2 text-sm text-ctp-text hover:border-ctp-blue">{user.displayName}</button>)}</div>}
    {selectedUser && <button type="button" onClick={() => { requestRef.current += 1; setSelectedUser(null); setResults([]); setStatus("ready"); setDeckError(null); }} className="mt-3 min-h-10 text-xs text-ctp-blue hover:underline">← Choose another user</button>}
    {selectedUser && status === "ready" && results.length === 0 && <InlineState className="mt-3">This user has no public decks.</InlineState>}
    {results.length > 0 && <div className="mt-3 space-y-2">{results.map((deck) => { const key = `user-${deck.publicSlug}`; const added = comparedKeys.has(key); return <div key={deck.publicSlug} className="flex items-center gap-3 rounded-lg border border-ctp-surface1 p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-ctp-text">{deck.title}</p><p className="text-xs text-ctp-subtext0">by {deck.owner.displayName}{deck.championName ? ` · ${deck.championName}` : ""}</p></div><button type="button" disabled={loadingSlug !== null} aria-pressed={added} onClick={() => void toggle(deck)} className={`min-h-10 shrink-0 rounded-md border px-3 py-1.5 text-xs disabled:opacity-50 ${added ? "border-ctp-blue text-ctp-blue" : "border-ctp-surface1 text-ctp-subtext1 hover:border-ctp-blue"}`}>{loadingSlug === deck.publicSlug ? "Loading…" : added ? "− Remove" : "+ Compare"}</button></div>; })}</div>}
  </div>;
}
