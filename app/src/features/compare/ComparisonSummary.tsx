    ...
    {mode === "overview" && summaries.map((summary, targetIndex) => {
      ...
      return <section key={summary.key} aria-labelledby={`analysis-${summary.key}`} className="space-y-5">
        ...
        {!summary.loading && !summary.unavailable && baselineStats && targetStats && <>
          ...
          <Section heading="dense" title="Key differences" actions={summary.changes.length > changes.length ? <button type="button" onClick={onViewAllDifferences} className="text-xs font-medium text-ctp-blue hover:underline">All {summary.changes.length} differences →</button> : undefined}>
          ...
          <details className="rounded-xl border border-ctp-surface1 bg-ctp-mantle">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-ctp-subtext1 hover:text-ctp-text">More comparison details</summary>
            <div className="space-y-4 border-t border-ctp-surface1 p-4"><div><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-ctp-text">Deck profile</h3>{(summary.championChanged || summary.spiritChanged) && <div className="text-xs text-ctp-subtext0">{summary.championChanged && <span>Champion: {summary.baselineChampion ?? "—"} → {summary.targetChampion ?? "—"}</span>}{summary.championChanged && summary.spiritChanged && <span className="mx-2">·</span>}{summary.spiritChanged && <span>Spirit: {summary.baselineSpirit ?? "none"} → {summary.targetSpirit ?? "none"}</span>}</div>}</div><DeckProfile baseline={baselineStats} target={targetStats} /></div>{baselineList && targetList && <DeckOverlapVisualization baseline={baselineList} target={targetList} cardsByName={cardsByName} onViewAll={onViewAllDifferences} />}</div>
          </details>
        </>}
      </section>;
    })}
