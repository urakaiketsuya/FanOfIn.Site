...
export default function PantheonDeckDetail() {
  ...
  if (deck === undefined) return <div className="mx-auto max-w-4xl px-4 py-8 text-ctp-subtext1">Loading Pantheon deck…</div>;
  if (deck === null) return <div className="mx-auto max-w-4xl px-4 py-8"><PageHeader title="Pantheon deck not found" /><Link to="/decks?view=pantheon" className="text-ctp-blue">← Browse Pantheon decks</Link></div>;
  ...
  return <div className="mx-auto max-w-3xl px-4 py-8">
    ...
    {tab === "decklist" && <div role="tabpanel" id="pantheon-deck-panel-decklist" aria-labelledby="pantheon-deck-tab-decklist"><div className="mt-6"><DecklistView decklist={decklist} cardsByName={cardsByName} showThumbnails defaultDisplayMode="compact" extraSections={[{ title: "Boons", lines: boons.map((line) => ({ card: line.name, quantity: line.quantity })) }]} trailingSections={[{ title: "Tokens", lines: tokens.map((line) => ({ card: line.name, quantity: line.quantity })) }]} /></div></div>}
