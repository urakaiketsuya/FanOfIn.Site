...
export default function PrepareAnalysis({ lines, catalogByName, profile, onRolesChange, onReviewed }: { lines: Line[]; catalogByName: Map<string, Card>; profile: DeckAnalysisProfile; onRolesChange: (roles: Record<string, GamePlanRole | "">) => void; onReviewed: () => void }) {
  ...
  return <details className="group rounded-xl border border-ctp-blue/40 bg-ctp-blue/5" open={!ready || !profile.reviewedAt}>
    ...
    <div className="border-t border-ctp-blue/20 p-3 sm:p-4">
      ...
      {profile.inheritedFrom && <p className="mt-2 rounded-lg border border-ctp-yellow/30 bg-ctp-yellow/5 p-2 text-xs text-ctp-subtext1">Carried unchanged card names forward from revision {profile.revision - 1}. Review this deck version before relying on its results.</p>}
      <div className="mt-3 grid gap-2 sm:grid-cols-2">{tools.map((tool) => <div key={tool.name} className="flex items-center justify-between gap-3 rounded-lg border border-ctp-surface1 bg-ctp-base/50 p-2"><span className="text-xs font-medium text-ctp-text">{tool.name}</span><span className={`text-[10px] font-semibold ${tool.ready ? "text-ctp-green" : "text-ctp-yellow"}`}>{tool.ready ? "Ready" : `Needs ${tool.missing}`}</span></div>)}</div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">{ready && <button type="button" onClick={onReviewed} className="min-h-11 rounded-lg bg-ctp-blue px-3 text-xs font-semibold text-ctp-base">{profile.reviewedAt ? "Reviewed" : "Mark reviewed"}</button>}{assigned > 0 && <button type="button" onClick={() => onRolesChange({})} className="min-h-11 rounded-lg px-3 text-xs font-medium text-ctp-red hover:bg-ctp-red/10">Clear classifications</button>}<span className="self-center text-[10px] text-ctp-subtext0">Profile revision {profile.revision}</span></div>
    </div>
  </details>;
}
