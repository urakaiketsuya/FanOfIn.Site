...
const CALCULATORS = ["Game plan readiness", "Opening hand recipe", "Level-up runway", "Pressure continuity", "Setup-to-payoff timing", "Interaction coverage", "Resilience and rebuild", "Test session tracker", "Draw quality by game stage", "Post-sideboard plan", "Card access and probability", "Clumping and conditional pressure", "Resource timing", "Advanced sequence analysis"];
...
export default function DeckAnalysisIndex() {
  ...
  return <PageLayout>
    ...
    {tab === "summary" && <div className="mt-4">
      ...
      <section className="mt-4 rounded-xl border border-ctp-surface1 bg-ctp-mantle p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-base font-semibold text-ctp-text">Deck calculator directory</h2><p className="mt-1 text-xs text-ctp-subtext0">Fourteen focused calculators cover consistency, pacing, resources, resilience, testing, and sideboarding.</p></div><button type="button" onClick={() => setTab("explore")} className="rounded-md bg-ctp-blue px-4 py-2 text-sm font-semibold text-ctp-base">Open calculators</button></div><div className="mt-3 flex flex-wrap gap-1.5">{CALCULATORS.map((name) => <span key={name} className="rounded-full border border-ctp-surface1 bg-ctp-base/40 px-2 py-1 text-[10px] text-ctp-subtext1">{name}</span>)}</div></section>
    ...
    {tab === "explore" && <div className="mt-4 space-y-3">
      ...
      <AnalysisDisclosure title="Setup-to-payoff timing" summary="Use two deadlines when setup must arrive before its payoff; use Game Plan Readiness for one shared checkpoint."><EnginePayoffBalance mainLines={workspace.main} materialLines={workspace.material} catalogByName={catalogByName} sharedAssignments={analysisRoles} onSharedAssignmentsChange={updateAnalysisRoles} /></AnalysisDisclosure>
