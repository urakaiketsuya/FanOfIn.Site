...
function EditableCardTile({ line, card, section, selected, onSelect, onChangeQuantity, onMove, onRemove }: { line: OmnidexDecklistCardLine; card: Card | undefined; section: DeckSectionKey; selected: boolean; onSelect: () => void; onChangeQuantity: (quantity: number) => void; onMove: (section: DeckCardDestination) => void; onRemove: () => void }) {
  ...
  return <article aria-label={`${line.quantity} copies of ${line.card} in ${section}`} className={`overflow-hidden rounded-xl border bg-ctp-mantle shadow-sm transition-[border-color,box-shadow] ${selected ? "border-ctp-blue ring-2 ring-ctp-blue/40" : "border-ctp-surface1"}`}>
    <div className="relative aspect-[5/7] bg-ctp-surface0">
      <CardHoverPreview artOnly image={card?.editions[0]?.image} backImage={reverseFace?.edition.image} backAlt={reverseFace?.name} alt={line.card}>
...
export function MaybeboardCardTile({ line, card, onChangeQuantity, onMove, onRemove }: { line: OmnidexDecklistCardLine; card: Card | undefined; onChangeQuantity: (quantity: number) => void; onMove: (destination: DeckCardDestination) => void; onRemove: () => void }) {
  ...
  return <div className="overflow-hidden rounded-lg border border-ctp-yellow/40 bg-ctp-mantle">
    <div className="relative aspect-[5/7] bg-ctp-surface0"><CardHoverPreview artOnly image={card?.editions[0]?.image} backImage={reverseFace?.edition.image} backAlt={reverseFace?.name} alt={line.card}>{card?.editions[0] ? <Link to={`/cards/${card.slug}`} className="block h-full w-full"><CardImage image={card.editions[0].image} alt={line.card} className="h-full w-full object-cover" /></Link> : <span className="flex h-full items-center justify-center p-2 text-center text-xs text-ctp-subtext0">{line.card}</span>}</CardHoverPreview><input type="number" min={1} max={maxQuantity} value={line.quantity} aria-label={`Maybeboard copies of ${line.card}`} onChange={(event) => { const quantity = Number(event.target.value); if (Number.isInteger(quantity) && quantity >= 1) onChangeQuantity(Math.min(quantity, maxQuantity)); }} className="absolute right-1.5 top-1.5 min-h-11 w-11 rounded border border-ctp-surface1 bg-ctp-base/90 px-1 text-right text-xs text-ctp-text" /></div>
...
export function EditableDecklistGrid({ decklist, cardsByName, onChangeQuantity, onAdjustSelected, onSetSelected, onMoveSelected, onRemoveSelected, onMove, onRemove }: { decklist: OmnidexDecklist; cardsByName: Map<string, Card>; onChangeQuantity: (section: DeckSectionKey, name: string, quantity: number) => void; onAdjustSelected: (cards: SelectedCard[], delta: number) => void; onSetSelected: (cards: SelectedCard[], quantity: number) => void; onMoveSelected: (cards: SelectedCard[], destination: DeckCardDestination) => void; onRemoveSelected: (cards: SelectedCard[]) => void; onMove: (from: DeckSectionKey, to: DeckCardDestination, name: string) => void; onRemove: (section: DeckSectionKey, name: string) => void }) {
    ...
    <div className="sticky top-2 z-20 flex flex-wrap items-center gap-2 rounded-xl border border-ctp-blue/40 bg-ctp-base/95 p-2.5 shadow-lg backdrop-blur">
      ...
      <details className="relative">
        ...
        <div className="absolute right-0 top-full z-30 mt-2 w-60 rounded-xl border border-ctp-surface1 bg-ctp-base p-3 shadow-xl">
          ...
          <div className="mt-2 grid grid-cols-4 gap-2">{[1, 2, 3, 4].map((quantity) => <button key={quantity} type="button" disabled={selectedCards.length === 0} onClick={() => onSetSelected(selectedCards, quantity)} className="min-h-11 rounded-lg border border-ctp-surface1 text-sm disabled:opacity-40">{quantity}</button>)}</div>
          <label className="mt-3 block text-[10px] font-semibold uppercase tracking-wide text-ctp-subtext0">Move selected<select disabled={selectedCards.length === 0} defaultValue="" onChange={(event) => { if (event.target.value) { onMoveSelected(selectedCards, event.target.value as DeckCardDestination); event.target.value = ""; } }} className="mt-1 min-h-11 w-full rounded-lg border border-ctp-surface1 bg-ctp-base px-2 text-xs font-normal normal-case text-ctp-text disabled:opacity-40"><option value="">Choose section…</option><DestinationOptions cards={selectedCards.map((selectedCard) => cardsByName.get(selectedCard.name))} /></select></label>
          <button type="button" disabled={selectedCards.length === 0} onClick={() => { if (window.confirm(`Remove ${selectedCards.length} selected card${selectedCards.length === 1 ? "" : "s"}?`)) { onRemoveSelected(selectedCards); setSelected(new Set()); } }} className="mt-3 min-h-11 w-full rounded-lg border border-ctp-red/50 text-xs text-ctp-red disabled:opacity-40">Remove selected</button>
        </div>
      </details>
    </div>
