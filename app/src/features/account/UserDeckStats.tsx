...
export default function UserDeckStats({ decklist, championName, format }: { decklist: OmnidexDecklist; championName: string | null; format: DeckFormat }) {
  ...
  return <div className="mt-6 space-y-6">
    <section className={`rounded-lg border p-4 ${validationTone}`}>
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">{validation.status}</h2><p className="mt-1 text-xs opacity-80">Main {totals.main} · Material {totals.material} · Sideboard {totals.sideboard}</p></div>{builderParams && <Link to={buildDeckBuilderPath(builderParams.championName, builderParams.spiritFilter, builderParams.lockedCards, builderParams.lockedSections)} className="rounded-md border border-current px-3 py-1.5 text-sm">Edit in Deck Builder</Link>}</div>
