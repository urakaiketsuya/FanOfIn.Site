...
export default function DeckVisualStrip({ decklist, championName }: { decklist: OmnidexDecklist; championName: string | null }) {
  ...
  return <div data-component="DeckVisualStrip" className="mt-4 flex h-24 gap-2 overflow-hidden rounded-lg bg-ctp-base p-2" aria-label="Featured deck cards">
    <CardHoverPreview image={championCard?.editions[0]?.image} backImage={championBack?.edition.image} backAlt={championBack?.name} alt={championName ?? "Unknown champion"}>
    ...
    <div className="grid min-w-0 flex-1 grid-cols-4 gap-1.5">{featuredLines.map((line) => {
      ...
      return <CardHoverPreview key={line.card} image={card?.editions[0]?.image} backImage={alternateFace?.edition.image} backAlt={alternateFace?.name} alt={line.card}>{card ? <Link to={`/cards/${card.slug}`} title={line.card} className="relative block h-20 overflow-hidden rounded bg-ctp-surface0">{content}</Link> : <div title={line.card} className="relative h-20 overflow-hidden rounded bg-ctp-surface0">{content}</div>}</CardHoverPreview>;
    })}</div>
  </div>;
}
