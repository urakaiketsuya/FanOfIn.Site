...
import type { AccountUser, DeckFormat, DeckImportPreview, SavedDeck } from "@gatcg/shared";
...
export default function MyDecksIndex() {
  ...
  const refreshDecks = useCallback(async () => setDecks((await accountApi.decks()).decks), []);
  ...
  if (!user) return <div className="mx-auto max-w-xl px-4 py-12"><h1 className="text-2xl font-bold text-ctp-blue">My Decks</h1><p className="mt-2 text-ctp-subtext1">Sign in to save decklists and keep imported tournament and community builds together.</p><div className="mt-6"><GoogleSignInButton onCredential={(credential) => void run(async () => { const session = await accountApi.googleSignIn(credential); setUser(session.user); await refreshDecks(); })} /></div>{error && <p className="mt-4 text-sm text-ctp-red">{error}</p>}</div>;

  return <div className="mx-auto max-w-4xl px-4 py-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-ctp-blue">My Decks</h1><p className="mt-1 text-sm text-ctp-subtext1">Signed in as {user.displayName} · {decks.length} unique build{decks.length === 1 ? "" : "s"}</p></div><button type="button" onClick={() => void run(async () => { await accountApi.logout(); setUser(null); setDecks([]); })} className="rounded-md border border-ctp-surface1 px-3 py-1.5 text-sm text-ctp-subtext1 hover:text-ctp-text">Sign out</button></div>
    ...
    <section className="mt-8"><h2 className="text-lg font-semibold">Saved builds</h2>{decks.length === 0 ? <p className="mt-3 rounded-lg border border-dashed border-ctp-surface1 p-6 text-center text-sm text-ctp-subtext1">Import or paste a decklist to start your library.</p> : <div className="mt-3 space-y-3">{decks.map((deck) => <details key={deck.id} className="rounded-xl border border-ctp-surface1 bg-ctp-mantle p-4"><summary className="cursor-pointer list-none"><div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold text-ctp-text">{deck.title}</h3><p className="text-xs text-ctp-subtext1">{deck.championName ?? "Unknown champion"} · {deck.format} · {deck.sources.length} source{deck.sources.length === 1 ? "" : "s"}</p></div><span className="text-xs text-ctp-subtext0">{deck.decklist.main.reduce((sum, line) => sum + line.quantity, 0)} main</span></div></summary><pre className="mt-4 max-h-72 overflow-auto whitespace-pre-wrap rounded-md bg-ctp-base p-3 text-xs text-ctp-subtext1">{buildDecklistText(deck.decklist)}</pre><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => { const next = window.prompt("Deck name", deck.title); if (next?.trim()) void run(async () => { await accountApi.renameDeck(deck.id, next); await refreshDecks(); }); }} className="rounded border border-ctp-surface1 px-2 py-1 text-xs">Rename</button><button type="button" onClick={() => { if (window.confirm(`Delete ${deck.title}?`)) void run(async () => { await accountApi.deleteDeck(deck.id); await refreshDecks(); }); }} className="rounded border border-ctp-red/60 px-2 py-1 text-xs text-ctp-red">Delete</button></div></details>)}</div>}</section>
  </div>;
}
