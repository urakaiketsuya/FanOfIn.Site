...
export default function PublicDeckDetail() {
  ...
  return <PageLayout data-component="PublicDeckDetail">
    ...
    <div className="mt-5 flex flex-wrap items-center gap-2">
      <Link to={`/deck-analysis?publicDeck=${encodeURIComponent(publicSlug)}`} className="rounded-md border border-ctp-surface1 px-3 py-1.5 text-sm font-medium text-ctp-subtext1 hover:border-ctp-blue hover:text-ctp-text">Analyze deck</Link>
      <Link to={`/deck-review?publicDeck=${encodeURIComponent(publicSlug)}`} className="rounded-md border border-ctp-surface1 px-3 py-1.5 text-sm font-medium text-ctp-subtext1 hover:border-ctp-blue hover:text-ctp-text">Review suggestions</Link>
      ...
      {!social && <Link to="/decks/edit" className="text-sm text-ctp-blue hover:underline">Sign in to like, favorite, or copy</Link>}
    </div>
