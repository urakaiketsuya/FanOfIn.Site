...
export default function AccountIndex() {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-8">
    ...
    <section className="mt-6 rounded-xl border border-ctp-red/40 bg-ctp-red/5 p-4"><h2 className="font-semibold text-ctp-red">Delete account</h2><p className="mt-1 text-sm text-ctp-subtext1">Permanently remove your account, decks, saved decks, and social activity.</p><button type="button" disabled={busy} onClick={() => { if (window.prompt("Permanently delete your account and saved decks? Type DELETE to confirm.") === "DELETE") void run(async () => { await accountApi.deleteAccount(); setUser(null); }); }} className="mt-3 rounded-md border border-ctp-red/60 px-3 py-1.5 text-sm text-ctp-red">Delete account</button></section>
  </div>;
}
