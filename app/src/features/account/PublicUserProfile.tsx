...
export default function PublicUserProfile() {
  ...
  if (!profile) return <PageLayout data-component="PublicUserProfile" width="wide"><EmptyState title="Profile unavailable" description={error} action={<Link to="/shared-decks" className="text-ctp-blue hover:underline">Browse shared decks</Link>} /></PageLayout>;
  return <PageLayout data-component="PublicUserProfile" width="wide"><Link to="/shared-decks" className="text-sm text-ctp-blue hover:underline">← Shared Decks</Link><h1 className="mt-4 text-3xl font-bold">{profile.displayName}</h1><p className="mt-1 text-ctp-subtext1">{profile.decks.length} public deck{profile.decks.length === 1 ? "" : "s"}</p>{profile.decks.length === 0 ? <EmptyState className="mt-8" title="This user has no public decks" /> : <div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{profile.decks.map((deck) => <PublicDeckCard key={deck.publicSlug} deck={deck} />)}</div>}</PageLayout>;
}
