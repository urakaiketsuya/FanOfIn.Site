...
function Chance({ label, value, emphasis = false }: { label: string; value: number; emphasis?: boolean }) { return <div><p className="text-[9px] uppercase tracking-wide text-ctp-subtext0">{label}</p><p className={`text-sm font-semibold tabular-nums ${emphasis ? "text-ctp-yellow" : "text-ctp-text"}`}>{(value * 100).toFixed(1)}%</p></div>; }
