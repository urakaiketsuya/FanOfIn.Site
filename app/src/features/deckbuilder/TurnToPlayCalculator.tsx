...
export default function TurnToPlayCalculator({ mainLines, catalogByName }: { mainLines: { name: string; quantity: number }[]; catalogByName: Map<string, Card> }) {
  ...
  const [seenAccelerantCount, setSeenAccelerantCount] = useState(-1);
  if (simpleAccelerants.length !== seenAccelerantCount) {
    setSeenAccelerantCount(simpleAccelerants.length);
  ...
  return (
    <Panel data-component="TurnToPlayCalculator" className="mt-4 shadow-sm">
      <Section
        heading="dense"
        title="Turns to play"
        description={<>Earliest turn a card's Reserve cost and champion-level requirement could both be met — a theoretical ceiling, not a guarantee (see notes below).</>}
      >
            ...
            <div className="mt-1.5 space-y-1">
              {simpleAccelerants.map((card) => (
                <label key={card.name} className="flex items-center gap-2 text-xs text-ctp-subtext1">
                  <input type="checkbox" checked={checkedAccelerants.has(card.name)} onChange={(e) => setCheckedAccelerants((current) => { const next = new Set(current); e.target.checked ? next.add(card.name) : next.delete(card.name); return next; })} />
        ...
        <div className="mt-4"><ForecastHeadline label="Earliest turn this card could be played" value={`Turn ${result.earliestTurn}`} /></div>
        ...
        <p className="mt-3 text-[10px] leading-4 text-ctp-subtext0">Assumes: a hand-size ceiling of starting hand + 1 card drawn per turn, ignoring anything spent that turn on other costs; that the deck has a champion print at every level up to the one required; and that every counted accelerant is actually drawn and its own Reserve cost is free to pay that turn. A real game is very likely to run later than this ceiling, never earlier.</p>
      </Section>
    </Panel>
  );
}
