...
export function applySimulatorEvidence(
  ...
  const rank = (a: SuggestedCard, b: SuggestedCard) => {
    ...
    const leftScore = ((left.winRate ?? 0.5) * left.games + 5) / (left.games + 10);
    const rightScore = ((right.winRate ?? 0.5) * right.games + 5) / (right.games + 10);
...
export function useSimulatorSuggestedBuild(
  ...
  return useMemo(() => applySimulatorEvidence(baseline, summary, catalog), [baseline, summary, catalog]);
}
