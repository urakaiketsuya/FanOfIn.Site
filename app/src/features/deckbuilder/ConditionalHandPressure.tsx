...
export default function ConditionalHandPressure({ mainLines, materialLines, catalogByName }: {
  ...
  return <Panel data-component="ConditionalHandPressure" className="mt-4 shadow-sm"><Section heading="dense" title="Conditional hand pressure" description="How often could multiple cards ask for an external condition at the same time?">
    ...
    <p className="mt-2 text-[10px] leading-4 text-ctp-subtext0">Exact without-replacement draw odds; no mulligan. This conservatively flags printed bonus, champion, board, graveyard, and opponent requirements. It does not prove a card is dead, evaluate whether its condition is already enabled, or count self-contained “if you do” effects.</p>
  </Section></Panel>;
}
