...
const STORAGE_KEY = "fanofin:match-log:v1";
...
export default function MatchLogIndex() {
  ...
  function addManual() {
    ...
    const record: MatchLogRecord = { version: 1, id: crypto.randomUUID(), playedAt: now, result, order, turns: turns ? Math.max(0, Math.floor(Number(turns))) : null, mulligans: mulligans ? Math.max(0, Math.floor(Number(mulligans))) : null, opponent: opponent.trim(), deckLabel: selectedDeck?.title ?? deckLabel.trim(), savedDeckId: selectedDeck?.id ?? null, sideboardPlan: sideboardPlan.trim(), gamePlanTurn: gamePlanTurn ? Math.max(1, Math.floor(Number(gamePlanTurn))) : null, notableCards: splitList(notableCards), bottlenecks: splitList(bottlenecks), notes: notes.trim(), provenance: { kind: "manual", enteredAt: now } };
    save([record, ...records]); setOpponent(""); setTurns(""); setMulligans(""); setSideboardPlan(""); setGamePlanTurn(""); setNotableCards(""); setBottlenecks(""); setNotes("");
  }
...
function ImportPreview({ value, cardOptions, onImport }: { value: { previews: ClarentImportPreview[]; errors: string[] }; cardOptions: { uuid: string; name: string }[]; onImport: (previews: ClarentImportPreview[]) => void }) {
  const [mappingNames, setMappingNames] = useState<Record<string, string>>({});
  const byName = useMemo(() => new Map(cardOptions.map((card) => [card.name.toLocaleLowerCase(), card])), [cardOptions]);
  ...
  const mappedPreviews = applyClarentCardMappings(value.previews, mappings);
  const fresh = mappedPreviews.filter((entry) => !entry.duplicate);
  return <div className="mt-3 rounded-lg border border-ctp-surface1 bg-ctp-base p-3"><h3 className="text-sm font-semibold text-ctp-text">Import preview</h3><datalist id="clarent-card-names">{cardOptions.map((card) => <option key={card.uuid} value={card.name} />)}</datalist>{value.errors.map((error) => <p key={error} className="mt-2 text-xs text-ctp-red">{error}</p>)}<div className="mt-2 space-y-2">{mappedPreviews.map((entry) => <div key={entry.record.id} className="rounded-lg border border-ctp-surface1 p-2 text-xs"><p className="font-medium text-ctp-text">{entry.record.deckLabel} vs. {entry.record.opponent} · {entry.record.result}</p><p className="mt-1 text-ctp-subtext0">{entry.duplicate ? "Already imported — will be skipped" : "Ready to import"} · {entry.record.provenance.kind === "clarent" ? entry.record.provenance.submissionId : ""}</p>{entry.unresolvedCardIds.length > 0 && <details className="mt-2"><summary className="cursor-pointer text-ctp-yellow">Resolve {entry.unresolvedCardIds.length} unknown card ID{entry.unresolvedCardIds.length === 1 ? "" : "s"}</summary><div className="mt-2 space-y-2">{entry.unresolvedCardIds.map((id) => <label key={id} className="block text-ctp-subtext0"><span className="block break-all">{id}</span><input list="clarent-card-names" value={mappingNames[id] ?? ""} onChange={(event) => setMappingNames((current) => ({ ...current, [id]: event.target.value }))} placeholder="Choose canonical card…" className="mt-1 min-h-11 w-full rounded-lg border border-ctp-surface1 bg-ctp-mantle px-3 text-sm text-ctp-text" /></label>)}</div></details>}</div>)}</div><button type="button" disabled={fresh.length === 0} onClick={() => onImport(mappedPreviews)} className="mt-3 min-h-11 w-full rounded-lg bg-ctp-mauve px-4 text-sm font-semibold text-ctp-base disabled:opacity-40 sm:w-auto">Import {fresh.length} new game{fresh.length === 1 ? "" : "s"}</button></div>;
}
