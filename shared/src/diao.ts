...
export function computeDeckRating(lines: NamedCardLine[], cards: Map<string, DiaoCard>, championName: string | null, championClasses: string[]): DeckRating {
  ...
  const signals: DeckRatingSignals = {
    ...
    repeatableDraw: 0, oneShotDraw: 0, floatingMemory: floatingMemory(lines, cards, championName, championClasses),
    banish: 0, destroy: 0, negate: 0, fastSpeed: 0, recover: 0, protection: 0, threats: 0,
  };
  let costSum = 0;
  let costN = 0;
  let allyPower = 0;
...
export function cardPillarScore(card: DiaoCard, pillar: RatingPillar): number {
  ...
  if (pillar === "opportunity") return /whenever .* draw a card|\[REST\].*draw a card|at the (beginning|start) of .* draw a card/i.test(effect) ? 4 : /draw (a|two|three|\d+) card/i.test(effect) ? 1.5 : 0;
  if (pillar === "interaction") return (!isChampion && /\bbanish\b/i.test(effect) ? 0.3 : 0) + (!isChampion && /\bdestroy\b/i.test(effect) ? 0.3 : 0) + (/\*\*negate\*\*/i.test(card.effect ?? "") ? 2 : 0) + (/fast activation/i.test(effect) ? 1.5 : 0);
  const recover = effect.match(/recover (\d+)/i);
  return (recover ? Number(recover[1]) * 0.3 : 0) + (/spellshroud|intercept|\bprevent\b/i.test(effect) ? 0.5 : 0) + (isAlly && card.power != null && card.power >= 2 ? 0.3 : 0);
}
